...
class JointSegmentView: UIView, AnimatedTransitioning {
    ...
    private func updatePathLayer() {
        ...
        // serve detection
        if let rightWristJoint = joints[VNHumanBodyPoseObservation.JointName.rightWrist], let rightShoulderJoint = joints[VNHumanBodyPoseObservation.JointName.rightShoulder] {
            ...
            if yDifference < -0.1 {
                if !serveDetected {
                    ...
                    serveDetected = true
                    AICoach.provideFeedback(for: joints, pose: "Hit behind")
        ...
        if let leftWristJoint = joints[VNHumanBodyPoseObservation.JointName.leftWrist], let leftShoulderJoint = joints[VNHumanBodyPoseObservation.JointName.leftShoulder] {
            ...
            if yDifference < -0.1 { // checking if the wrist is above the shoulder
                if !trophyDetected {
                    ...
                    trophyDetected = true
                    AICoach.provideFeedback(for: joints, pose: "Trophy behind")
